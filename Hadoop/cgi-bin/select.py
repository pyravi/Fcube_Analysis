...
for i in range(0,len(ip)) :
	if ip[i] != '' and ip[i] != "192.168.126.1" and ip[i] != "192.168.126.2" and ip[i] != "192.168.126.254" and ip[i] != "main":
		#os.system("sudo sshpass -p '"+mp+"' ssh "+ip[i]+" yum install hadoop -y >> /var/www/html/in.txt")
		#os.system("sudo sshpass -p '"+mp+"' ssh "+ip[i]+" yum install jdk -y >> /var/www/html/in.txt")
